...
- `provider` provides information about the provider client that sent this payload.
    ...
    - `type` shall be either `network` or `other` to indicate the kind of provider client. This determines which client has priority when using a feature that is supported by multiple clients, as networks will have priority in such cases ([learn more]()).
    - `features` shall be an array with with all features supported by the provider client. A list of features can be found on the [TODO]() page. A provider client providing an empty features array may not receive any requests.
...
2. Provider client responds with the following JSON payload:
    ...
    The `provider.type` field shall be populated with either `network` or `other` to indicate the kind of provider client. This determines which client has priority when using a feature that is supported by multiple clients, as networks will have priority in such cases ([learn more]()).

    The `provider.features` field shall be populated with a list of all features supported by the provider client. A list of features can be found on the [TODO]() page.
</Steps>
